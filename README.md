...
Когда активный слой заполняется, автоматически вытесняется нейрон с самым старым `LastSeen` (политика LRU). Поиск через `Lookup` / `GetByID` обновляет `LastSeen`, поэтому часто используемые записи дольше остаются активными.
...
// Найти по ID (также обновляет LastSeen для активных)
neuron, loc = r.GetByID("task-42")
...
| `Add(n Neuron) error` | Добавить нейрон. При заполнении вытесняет LRU-жертву |
| `Lookup(title, theme string) (*Neuron, Location)` | Найти нейрон по заголовку и теме. Обновляет `LastSeen` в активном слое |
| `GetByID(id string) (*Neuron, Location)` | Найти нейрон по ID. Обновляет `LastSeen` в активном слое |
...
| `Edges() []Edge` | Снимок связей |
| `Save(path string) error` | Атомарно сохранить в JSON-файл (temp + `Rename`) |
| `LoadRegion(path string) (*Region, error)` | Загрузить из JSON-файла (валидирует `maxNeurons`) |
